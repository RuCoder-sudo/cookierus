...
<style>
    ...
    <?php if (!empty($banner['is_blocking'])): ?>
    .cookierus-overlay {
        position: fixed;
    ...
    .cookierus-overlay #cookierus-banner {
...
">
    <div class="cookierus-content-wrapper">
        <?php if (!empty($banner['title'])): ?>
            <h4 class="cookierus-title" style="text-align: <?php echo esc_attr($banner['text_align'] ?? 'left'); ?>; margin-bottom: 8px;"><?php echo esc_html($banner['title']); ?></h4>
        <?php endif; ?>
        <div class="cookierus-text" style="text-align: <?php echo esc_attr($banner['text_align'] ?? 'left'); ?>;">
    ...
    <div class="cookierus-buttons" style="justify-content: <?php echo esc_attr($banner['btn_position'] ?? 'flex-start'); ?>;">
...
(function() {
    function init() {
        ...
        if (localStorage.getItem('cookierus_consent')) {
            ...
            banner.remove(); // Completely remove element from DOM so it never flashes on load
            return;
        }
        ...
        function logConsent(status, categories = 'all') {
            ...
            // Also set a real cookie so PHP knows not to render it at all
            document.cookie = "cookierus_consent=" + status + "; max-age=" + (60*60*24*365) + "; path=/; SameSite=Lax";
            ...
            const overlay = document.querySelector('.cookierus-overlay');
            if (overlay) {
