...
- **Исправлено:** удалены внешние ссылки и значения URL политики из кода, настроек миграции и документации.
- **Добавлено:** JivoSite (онлайн-чат) с загрузкой только после согласия на аналитические cookie.
- **Совместимость:** метаданные обновлены для WordPress 7.1.

### Изменения в версии 1.1.8
- **Исправлено:** выключенные аналитические и маркетинговые сервисы сохраняются выключенными и не возвращаются после обновления страницы.
- **Исправлено:** вкладка «Цели обработки» использует сохранённые настройки администратора; добавлена цель «Ограниченная реклама».
- **Добавлено:** отдельное предупреждаемое разрешение на загрузку аналитики до согласия пользователя. Маркетинговые сервисы остаются заблокированными.
- **Добавлено:** загрузка Mail.ru Top по ID, обновлены инструкции для Яндекс.Метрики и Callibri.
- **Добавлено:** положение баннера «По центру (popup)» и переключатель затемнения страницы.

### Изменения в версии 1.1.7
- **Исправлено:** критический сбой фронтенда при рендеринге баннера после обновления до 1.1.6.
- **Изменено:** проверка нового тумблера выполняется напрямую по сохранённым настройкам без вызова дополнительного метода класса.
- **Улучшено:** совместимость с частично обновлёнными файлами и старым OPcache.

### Изменения в версии 1.1.6
- **Добавлено:** отдельный тумблер «Ограничить регистрацию и вход российскими email-доменами».
- **Изменено:** ограничение по умолчанию выключено, чтобы существующие сайты и администраторы не теряли доступ после обновления.
- **Исправлено:** вход по имени пользователя больше не проверяется по email, сохранённому в профиле.
- **Добавлено:** администраторы могут входить по иностранному email даже при включённом ограничении.
- **Изменено:** вкладка «Упоминания» показывает фактическое состояние настройки.

### Изменения в версии 1.1.5
- **Исправлено:** после нажатия «Принять все» страница обновляется, поэтому Яндекс.Метрика и Callibri корректно догружаются и инициализируются.
- **Изменено:** кнопка «Отозвать согласие» и её настройка полностью убраны из интерфейса сайта и админки.
- **Добавлено:** ссылка `/?cookierus_revoke=1` для размещения в политике конфиденциальности и резервный переход на неё при пустом URL отказа.
- **Изменено:** URL страницы политики по умолчанию больше не задаётся внешним адресом; владелец сайта указывает собственную страницу.
